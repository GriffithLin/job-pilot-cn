...
1. **落档**：原始 JD 逐字保存到用户笔记仓库的 JD 目录（具体路径按当前运行环境/adapter
   配置，不写死），文件名 `JD-<公司或平台>-<主题>-<YYYYMMDD>.md`
...
- **单 JD 映射表**：JD 要求 → P1/P2/P3 分级 → 用户现状（✅/◐/❌）→ 差距动作，一张四列表
- **多帖/多岗汇总**：每岗一段小结（公司 / 岗位 / 关键词 / 薪资），尾部横向统计表（高频关键词 × 出现次数 × 典型薪资带）
